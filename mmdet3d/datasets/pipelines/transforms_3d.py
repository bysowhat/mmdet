...
@PIPELINES.register_module()
class ObjectRangeFilterMM(object):
    ...
    def filter_cam(self, input_dict):
      labels_2d_list = input_dict['gt_labels2d_cam_list']
      bboxes_2d_list = input_dict['gt_bboxes2d_cam_list']
      labels_3d_list = input_dict['gt_labels_3d_cam_list']
      bboxes_3d_list = input_dict['gt_bboxes_3d_cam_list']
      centers2d_list = input_dict['centers2d_cam_list']
      depths_list = input_dict['depths_cam_list']
      
      cam_mask = input_dict['camera_mask']
      num_cam = len(bboxes_2d_list)
      ...
      for i in range(num_cam):
        labels_2d = labels_2d_list[i]
        bboxes_2d = bboxes_2d_list[i]
        labels_3d = labels_3d_list[i]
        bboxes_3d = bboxes_3d_list[i]
        centers2d = centers2d_list[i]
        depths = depths_list[i]

        if cam_mask[i]:
            # mask = bboxes_3d.in_range_bev(self.range_cam)
            ...
            # limit rad to [-pi, pi]
            bboxes_3d.limit_yaw(offset=0.5, period=2 * np.pi)
        
        labels_2d_list_new.append(labels_2d)
        bboxes_2d_list_new.append(bboxes_2d)
        labels_3d_list_new.append(labels_3d)
        bboxes_3d_list_new.append(bboxes_3d)
        centers2d_list_new.append(centers2d)
        depths_list_new.append(depths)
      
      input_dict['gt_labels2d_cam_list'] = labels_2d_list_new
      input_dict['gt_bboxes2d_cam_list'] = bboxes_2d_list_new
      input_dict['gt_labels_3d_cam_list'] = labels_3d_list_new
      input_dict['gt_bboxes_3d_cam_list'] = bboxes_3d_list_new
      input_dict['centers2d_cam_list'] = centers2d_list_new
      input_dict['depths_cam_list'] = depths_list_new

    def filter_bev(self, input_dict):
      gt_bboxes_3d = input_dict['gt_bboxes_3d_bev']
      gt_labels_3d = input_dict['gt_labels_3d_bev']
      mask = gt_bboxes_3d.in_range_bev(self.range_bev)
      gt_bboxes_3d = gt_bboxes_3d[mask]
      # mask is a torch tensor but gt_labels_3d is still numpy array
      # using mask to index gt_labels_3d will cause bug when
      # len(gt_labels_3d) == 1, where mask=1 will be interpreted
    ...
    def __call__(self, input_dict):
        ...
        if isinstance(input_dict['gt_bboxes_3d_bev'],
                      (LiDARInstance3DBoxes, DepthInstance3DBoxes)):
            self.range_bev = self.pcd_range[[0, 1, 3, 4]]
        if isinstance(input_dict['gt_bboxes_3d_cam_list'][0], CameraInstance3DBoxes):
            self.range_cam = self.pcd_range[[0, 2, 3, 5]]
        
        self.filter_cam(input_dict)
        self.filter_bev(input_dict)
