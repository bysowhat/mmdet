...
class Collect3DMM(Collect3D):
    ...
    def __init__(
        ...
        keys,
        meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                   'depth2img', 'cam2img', 'cam2img_ori', 'pad_shape', 'scale_factor', 'flip',
                   'pcd_horizontal_flip', 'pcd_vertical_flip', 'box_mode_3d',
                   'box_type_3d', 'img_norm_cfg', 'pcd_trans', 'sample_idx',
                   'pcd_scale_factor', 'pcd_rotation', 'pcd_rotation_angle',
                   'pts_filename', 'transformation_3d_flow', 'trans_mat',
                   'affine_aug', 'box_type_3d_lidar', 'box_mode_3d_lidar',
                   'box_type_3d_cam', 'box_mode_3d_cam')):
        self.keys = keys
        self.meta_keys = meta_keys


@PIPELINES.register_module()
class DefaultFormatBundle3DMM(object):
    """Default formatting bundle.
    ...
    def __super_call__(self, results):
      if 'img' in results:
          if isinstance(results['img'], list):
              # process multiple imgs in single frame
              imgs = [img.transpose(2, 0, 1) for img in results['img']]
              ...
              results['img'] = DC(to_tensor(img), stack=True)
      for key in [
              'proposals', 'gt_bboxes_ignore', 'gt_labels',
              'gt_labels_3d', 'attr_labels', 'pts_instance_mask',
              'pts_semantic_mask', 'gt_bboxes2d_cam_list',
              'gt_labels2d_cam_list', 'centers2d_cam_list', 'depths_cam_list',
              'gt_labels_3d_cam_list', 'gt_labels_3d_bev', 'coords_list'
      ...
      if 'gt_bboxes_3d_bev' in results:
        key = 'gt_bboxes_3d_bev'
    ...
    def __call__(self, results):
        ...
        results = self.__super_call__(results)
