...
from ..core.bbox import CameraInstance3DBoxes, LiDARInstance3DBoxes, get_box_type
...
class NuScenesMMDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        self.box_type_3d_cam, self.box_mode_3d_cam = get_box_type('camera')
        self.box_type_3d_lidar, self.box_mode_3d_lidar = get_box_type('lidar')
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type in self.cam_orders:
                ...
                coord_path = cam_info['coord_path']
        ...
        if not self.test_mode:
            annos_bev = self.get_bev_ann_info(index)
            input_dict['ann_info_bev'] = annos_bev
            if self.filter_empty_gt and annos_bev['gt_labels_3d'].shape[0] == 0:
                return None
            
            annos_cam = self.get_cam_ann_info(index)
            input_dict['ann_info_cam'] = annos_cam

        return input_dict
    
    def get_bev_ann_info(self, index):
        ...
        gt_bboxes_3d = LiDARInstance3DBoxes(
            ...
            origin=(0.5, 0.5, 0.5)).convert_to(self.box_mode_3d_lidar)
    ...
    def get_cam_ann_info(self, index):
        """Get annotation info according to the given index.
            For each cam view only.
        ...
        gt_names_3ds = []
        gt_labels_3ds = []
        # False means contain no gt
        camera_mask = []
        # Cam coord
        for cam_type in self.cam_orders:
            gt_bboxes_3d = info['cams'][cam_type]['bbox3ds']
            gt_names_3d = info['cams'][cam_type]['category_names']
            gt_center_2d = info['cams'][cam_type]['center2ds']
            gt_bboxes_2d = info['cams'][cam_type]['bbox2ds']
            gt_bboxes_depth = info['cams'][cam_type]['depths']
            # filter out bbox containing no points
            if self.use_valid_flag:
                mask = info['valid_flag']
                gt_bboxes_3d = gt_bboxes_3d[mask]
                gt_names_3d = gt_names_3d[mask]
                gt_center_2d = gt_center_2d[mask]
                gt_bboxes_2d = gt_bboxes_2d[mask]
                gt_bboxes_depth = gt_bboxes_depth[mask]
            gt_labels_3d = []
            for cat in gt_names_3d:
                if cat in self.CLASSES:
                    gt_labels_3d.append(self.CLASSES.index(cat))
            ...
            if gt_labels_3d.shape[0] != 0:
                gt_bboxes_3d = CameraInstance3DBoxes(
                    gt_bboxes_3d,
                    box_dim=gt_bboxes_3d.shape[-1],
                    origin=(0.5, 0.5, 0.5)).convert_to(self.box_mode_3d_cam)
                camera_mask.append(True)
            else:
                #Empty CameraInstance3DBoxes
                gt_bboxes_3d = CameraInstance3DBoxes(
                        gt_bboxes_3d,
                        box_dim=gt_bboxes_3d.shape[-1],
                        origin=(0.5, 1.0, 0.5))
            ...
            gt_bboxes_2ds.append(gt_bboxes_2d)
            gt_center_2ds.append(gt_center_2d)
            gt_bboxes_depths.append(gt_bboxes_depth)
            gt_names_3ds.append(gt_names_3d)
            gt_labels_3ds.append(gt_labels_3d)


        anns_results = dict(
            gt_bboxes_3d=gt_bboxes_3ds,
            gt_bboxes_2d=gt_bboxes_2ds,
            gt_center_2d=gt_center_2ds,
            gt_bboxes_depth=gt_bboxes_depths,
            gt_labels_3d=gt_labels_3ds,
            gt_names=gt_names_3ds,
    ...
    def pre_pipeline(self, results):
        ...
        results['mask_fields'] = []
        results['seg_fields'] = []
        results['box_type_3d_lidar'] = self.box_type_3d_lidar
        results['box_mode_3d_lidar'] = self.box_mode_3d_lidar
        results['box_type_3d_cam'] = self.box_type_3d_cam
        results['box_mode_3d_cam'] = self.box_mode_3d_cam
    ...
    def get_attr_name(self, attr_idx, label_name):
        ...
        if label_name == 'car' or label_name == 'bus' \
            ...
            if AttrMapping_rev2[attr_idx] == 'vehicle.moving' or \
                ...
                return NuScenesMMDataset.DefaultAttribute[label_name]
