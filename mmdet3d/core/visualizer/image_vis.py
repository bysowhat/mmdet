...
def map_pointcloud_to_image(points, 
    ...
    points = view_points(points[:3, :], camera_intrinsic, normalize=True)# points on image
    ...
    points = points[:, mask]
    points_lidar = points_lidar[mask, :2]
    coloring = coloring[mask]

    # Points near cam will cover points far from cam, if these two points projected to same pix.
    sort_index = np.argsort(coloring)[::-1]
    coloring = coloring[sort_index]
    points = points[:, sort_index]
    points_lidar = points_lidar[sort_index, :]
    depth_map = np.zeros((img.shape[0], img.shape[1]), dtype=np.float32)
    # coord_map: (n, 4) float
    # 4: x in origin image width, y in origin img height, x in lidar(m), y in lidar(m)
    coord_map_img = []
    coord_map_pts = []
    xs = np.minimum((points[0,:] + 0.5).astype(np.int32), depth_map.shape[1])
    ys = np.minimum((points[1,:] + 0.5).astype(np.int32), depth_map.shape[0])
    for x, y, c, coord_lidar in zip(xs, ys, coloring, points_lidar):
        depth_map[y, x] = c
    
    xs = points[0,:]
    ys = points[1,:]
    for x, y, coord_lidar in zip(xs, ys, points_lidar):
        coord_map_img.append(np.asarray((x, y)))
        coord_map_pts.append(coord_lidar)
    ...
    coord_map_img = np.asarray(coord_map_img)
    coord_map_pts = np.asarray(coord_map_pts)
    coord_map = np.concatenate([coord_map_img, coord_map_pts], 1)

    return depth_map, coord_map
