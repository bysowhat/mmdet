...
@BBOX_CODERS.register_module()
class FCOS3DBBoxCoderPts(FCOS3DBBoxCoder):
    """Bounding box coder for FCOS3D.
    ...
    def __init__(self,
                 ...
                 code_size=8,
                 norm_on_bbox=True):
        super(FCOS3DBBoxCoderPts, self).__init__(base_depths=base_depths,
        base_dims=base_dims,
        code_size=code_size,
        norm_on_bbox=norm_on_bbox)
    
    def decode(self, bbox, scale, offset, stride, training, cls_score=None):
        """Decode regressed results into 3D predictions.
           delta_xs, delta_ys, gt_h, gt_ry
        ...
        # scale the bbox of different level
        # only apply to offset, depth and size prediction
        scale_size_z, _, scale_size_h = scale[0:3]
        offset_size_z = offset[0]

        clone_bbox = bbox.clone()
        bbox[:, 0] = scale_size_z(clone_bbox[:, 0]).float()
        bbox[:, 0] = offset_size_z(bbox[:, 0]).float()
        bbox[:, 1] = scale_size_h(clone_bbox[:, 1]).float()
        bbox[:, 1] = bbox[:, 1].exp()
        if self.base_dims is not None:
