...
@VOXEL_ENCODERS.register_module()
class AVODBEV(nn.Module):
    """Voxel feature encoder used in AVOD.
    ...
    def __init__(self,
                 ...
                 fusion_layer=None,
                 return_point_feats=False):
        super(AVODBEV, self).__init__()
        self.in_channels = in_channels
        self.return_point_feats = return_point_feats
        self.fp16_enabled = False
        self.height_dim = height_dim
        self.reflect_dim = reflect_dim
    ...
    def get_heights(self):
      hd = self.height_dim
      min_h = self.point_cloud_range[hd]
      max_h = self.point_cloud_range[hd+3]
      step = self.vz
      assert 0 == (max_h - min_h) % step
      self.heights = [h for h in range(min_h, max_h+1, step)]
    ...
    def forward(self,
        ...
        Returns:
            tuple: If `return_point_feats` is False, returns voxel features and
                ...
                feature of each points inside voxels.
        """
        #If num of points in one voxel is less then 8, will pad (8-num of points)'s (0,0,0,0) to 8
        pad_mask = features.sum(dim=2, keepdim=True)==0
        pad_mask = pad_mask.type_as(features)
        pad_mask2 = (pad_mask*(-99.)).tile(features.shape[-1])
        pad_mask_features = features + pad_mask2

        points_height_max_index = pad_mask_features[:, :, self.height_dim].max(dim=1, keepdim=True).indices
        points_reflect_max = features[:, :, 3].gather(1,points_height_max_index)
        points_height_max = features[:, :, 2].gather(1,points_height_max_index)
        for i in range(len(self.heights)-1):
          mask0 = points_height_max >= self.heights[i]
          mask1 = points_height_max < self.heights[i+1]
          mask = torch.logical_and(mask0, mask1)
          points_height_max[mask] -= self.heights[i]

        points_density = (1.0-pad_mask).sum(dim=1, keepdim=False)
        points_density = torch.log(points_density+1)/torch.log(torch.tensor(features.shape[1], requires_grad=False))
        
        # Combine together feature decorations
        voxel_feats = torch.cat([points_reflect_max, points_density], dim=-1)

        return voxel_feats, points_height_max