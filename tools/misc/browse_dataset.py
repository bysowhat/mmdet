...
def parse_args():
    ...
    parser.add_argument(
        '--task',
        type=str,
        choices=['det', 'seg', 'multi_modality-det', 'mono-det', 'fusion-det'],
...
def show_fusion_det_data(input, out_dir, show=False):
    """Visualize 3D point cloud and 3D bboxes."""
    img_metas = input['img_metas']._data
    points = input['points']._data.numpy()
    gt_bboxes = input['gt_bboxes_3d_bev']._data.tensor
    if img_metas['box_mode_3d_lidar'] != Box3DMode.DEPTH:
...
def _show_fusion_proj_bbox_img(input, out_dir, index, show=False):
    """Visualize 3D bboxes on 2D image by projection."""
    gt_bboxes = input['gt_bboxes_3d_cam_list']._data[index]
    img_metas = input['img_metas']._data
    img = input['img']._data.numpy()[index]
    # need to transpose channel to first dim
    img = img.transpose(1, 2, 0)
    # no 3D gt bboxes, just show img
    if gt_bboxes.tensor.shape[0] == 0:
        gt_bboxes = None
    filename = Path(img_metas['filename'][index]).name
...
def get_frame_name(input):
    fram_name = Path(input['img_metas']._data['filename'][0]).name
    start = fram_name.index('__')
    end = fram_name[start+2:].index('__') + start + 2
    return fram_name[:start]+fram_name[end:]

def show_fusion_proj_bbox_img(input, out_dir, show=False, num_views=6):
    """Visualize 3D bboxes on 2D image by projection."""
    frame_name = get_frame_name(input)
    out_dir = os.path.join(out_dir, frame_name)
    for i in range(num_views):
        _show_fusion_proj_bbox_img(input, out_dir, i, show=show)
...
def main():
    ...
    for input in dataset:
        if vis_task == 'fusion-det':
            show_fusion_det_data(input, args.output_dir, show=args.online)
            show_fusion_proj_bbox_img(
                input,
                args.output_dir,
                show=args.online,
